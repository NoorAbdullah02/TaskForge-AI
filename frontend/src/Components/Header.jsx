          ...
          {isLoggedIn && (
            <nav className="hidden lg:flex items-center flex-1 min-w-0">
              <div className="flex items-center gap-1 xl:gap-1.5 px-1 w-full">
                ...
                {/* Secondary Items (visible on 2xl+) */}
                {secondaryItems.map(({ to, label, icon: Icon }) => {
                  ...
                  return (
                    <Link
                      ...
                      to={to}
                      className={[
                        'hidden 2xl:flex items-center gap-2 px-2 xl:px-2.5 2xl:px-3 py-2 rounded-xl text-[12.5px] font-semibold whitespace-nowrap transition-all',
                        active
                          ? 'bg-blue-600 text-white shadow-sm shadow-blue-500/30'
                          : 'text-slate-500 hover:text-slate-900 hover:bg-slate-100',
                      ].join(' ')}
                ...
                {/* 'More' dropdown button (visible only on lg, hidden on 2xl) */}
                {secondaryItems.length > 0 && (
                  <div className="relative hidden lg:flex 2xl:hidden" ref={moreMenuRef}>
                    <button
